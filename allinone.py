...
models_and_params = {
    "KNN": (
        ...
        StratifiedKFold(n_splits=5, shuffle=True, random_state=42)  # Klassenbalance wichtig
    ),
    "LogisticRegression": (
    ...
    "DecisionTree": (
        ...
        KFold(n_splits=5, shuffle=True, random_state=42)  # Baum-Modelle brauchen keine Stratifikation
    ),
    "RandomForest": (
    ...
    "GaussianNB": (
        GaussianNB(),
        {},
        LeaveOneOut()  # Für kleine Datensätze maximale Nutzung
    ),
    "MLPClassifier": (
...
for name, (model, param_dist, cv_strategy) in models_and_params.items():
    ...
    pipeline = Pipeline([
        ('scaler', StandardScaler()),  # Skalierung für alle Modelle
        ('model', model)
    ])
